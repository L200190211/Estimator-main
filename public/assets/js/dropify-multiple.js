...
(function (root, factory) {
    ...
    DropifyMultiple.prototype.readFile = function (input) {
        ...
        for (var i = 0; i < j; i++) {
            ...
            if (
                ...
                file.size < this.sizeToByte(this.settings.maxFileSizePreview)
            ) {
                ...
                reader.onload = (function (file) {
                    return function (_file) {
                        ...
                        image.onload = function () {
                            ...
                            var fIndex = _this.findFileIndexBasedOnName(
                                this.name
                            );

                            _this.setFileDimensions(
                                this.width,
                                this.height,
                                fIndex
                            );
    ...
    DropifyMultiple.prototype.onFileReady = function (event, previewable, img) {
        ...
        if (this.errorsEvent.errors.length === 0) {
            ...
            if (fileIndex >= 0) this.setPreview(previewable, img.src, img.name);
        } else {
     ...
     * @param {Int} height
     */
    DropifyMultiple.prototype.setFileDimensions = function (
        width,
        height,
        fileIndex
    ) {
    ...
    DropifyMultiple.prototype.findFileIndexBasedOnName = function (name) {
        for (var i = 0; i < this.files.length; i++) {
            if (this.files[i].name == name) return i;
        }
        return -1;
    };
     ...
     * @param {String} src
     */
    DropifyMultiple.prototype.setPreview = function (
        previewable,
        src,
        fileName
    ) {
    ...
    DropifyMultiple.prototype.isImage = function (fileName) {
        if (
            this.settings.imgFileExtensions.indexOf(
    ...
    DropifyMultiple.prototype.checkFileSize = function (fSize) {
        if (
            ...
            fSize > this.sizeToByte(this.settings.maxFileSize)
        ) {
    ...
    DropifyMultiple.prototype.validateImage = function (fileIndex) {
        ...
        if (
            ...
            this.settings.minWidth >= fObj.width
        ) {
        ...
        if (
            ...
            this.settings.maxWidth <= fObj.width
        ) {
        ...
        if (
            ...
            this.settings.minHeight >= fObj.height
        ) {
        ...
        if (
            ...
            this.settings.maxHeight <= fObj.height
        ) {
            this.pushError("maxHeight");
        }

        if (
            this.settings.allowedFormats.indexOf(this.getImageFormat(fObj)) ==
            "-1"
        ) {
